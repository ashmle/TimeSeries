...
import six
import pandas as pd
import ts_profiling.formatters as formatters
import ts_profiling.templates as templates
import ts_profiling.plot as plot
...
def to_html(sample, stats_object):
    ...
    if not isinstance(stats_object, dict):
        raise TypeError("stats_object must be of type dict. Did you generate this using the ts_profiling.describe() function?")

    if not set({'table', 'variables', 'freq', 'correlations'}).issubset(set(stats_object.keys())):
        raise TypeError(
            "stats_object badly formatted. Did you generate this using the ts_profiling.describe() function?")
